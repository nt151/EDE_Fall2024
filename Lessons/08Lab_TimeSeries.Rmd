...
BR_Rain_Decompos <- stl(Rainfall_ts, s.window = "periodic")


plot(BR_Rain_Decompos)

BR_Temp_Decompos<- stl (Temperature_ts, s.window = "periodic")
plot (BR_Temp_Decompos)
...
Rainall_decomp_Components <- as.data.frame(BR_Rain_Decompos$time.series[,1:3])


Rainall_decomp_Components <- mutate(Rainall_decomp_Components,
        Observed = BR_complete$Rainfall,     
        Date = BR_complete$Date)

Temp_decomp_Components <- as.data.frame(BR_Temp_Decompos$time.series[,1:3])


Temp_decomp_Components <- mutate(BR_Temp_Decompos,
